...
class CustomEnvironment extends NodeEnvironment {
  ...
  async teardown(): Promise<void> {
    ...
    if (this.global.__GETNX_OPEN_ROOT_LMDBS) {
      if (isWindows) {
        for (const rootDb of this.global.__GETNX_OPEN_ROOT_LMDBS.values()) {
        ...
        for (const [
          dbPath,
          rootDb,
        ] of this.global.__GETNX_OPEN_ROOT_LMDBS.entries()) {
      ...
      this.global.__GETNX_OPEN_ROOT_LMDBS = undefined
    }
